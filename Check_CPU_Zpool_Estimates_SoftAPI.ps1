...
$zergPoolStatus = Invoke-RestMethod http://www.zpool.ca/api/status #-UseBasicParsing -Headers @{"Cache-Control" = "no-cache"} | ConvertFrom-Json
$zergPoolValues = $zergPoolStatus.psobject.Properties.value
$zergPoolCurrencies = Invoke-RestMethod http://www.zpool.ca/api/currencies #-UseBasicParsing -Headers @{"Cache-Control" = "no-cache"} | ConvertFrom-Json
